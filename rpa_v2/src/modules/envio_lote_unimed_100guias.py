...
class UnimedUploader(BaseModule):
    ...
    def clicar_enviar_upload(self):
        ...
        time.sleep(2)
        try:
            # Aguarda o formulário de erro aparecer (até 10s)
...
class XMLGeneratorAutomation(BaseModule):
    ...
    def carregar_ate_100_guias(self):
        log_message("Carregando 100 registros", "INFO")
        for _ in range(4):  # Já carrega 20, então mais 4 vezes para chegar a 100
            self.driver.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            try:
                # Aguarda o spinner aparecer e sumir
                self.wait.until(
                    EC.visibility_of_element_located((By.CSS_SELECTOR, "span.paginationLoadingSpan svg.fa-spinner"))
                )
                self.wait.until_not(
                    EC.visibility_of_element_located((By.CSS_SELECTOR, "span.paginationLoadingSpan svg.fa-spinner"))
                )
    ...
    def marcar_todos_checkbox(self):
        checkbox = self.wait.until(
            EC.element_to_be_clickable((By.ID, "checkTodosPreFaturar"))
        )
        checkbox.click()
    ...
    def clicar_botao_situacao_faturamento(self):
        ...
        botao.click()
        # Aguarda e clica no botão "Confirmar faturamento"
        confirmar_btn = self.wait.until(
            EC.element_to_be_clickable((By.ID, "executarMudancaSitFaturamento"))
        )
        confirmar_btn.click()
    ...
    def executar_processo_completo_login_navegacao(self, unimed_user, unimed_pass, cancel_flag=None):
        try:
            ...
            self.carregar_ate_100_guias()
            self.marcar_todos_checkbox()
            if cancel_flag and cancel_flag.is_set():
                log_message("Execução cancelada pelo usuário.", "WARNING")
                self.fechar_navegador()
                return False
